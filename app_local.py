...
app = FastAPI(title="Document Parser API (Local)", version="2.0")
...
async def parse_passport(image: Image.Image) -> Dict[str, Any]:
    ...
    2. OCR отключен для локального тестирования
    """
    result = {
        "success": False,
    ...
    # OCR отключен для локального тестирования
    result.update({
        "success": False,
        "method": "mrz_only",
        "confidence": 0,
...
async def parse_driver_license(image: Image.Image) -> Dict[str, Any]:
    ...
    result.update({
        ...
        "confidence": 0,
        "data": {},
        "error": "PDF417 не найден. Для полного функционала установите Tesseract OCR"
    })
    return result
...
@app.get("/health")
def health_check():
    return {"status": "ok", "service": "document-parser-local"}

@app.get("/")
def root():
    return {
        "message": "Document Parser API (Local Version)",
        "version": "2.0",
        "endpoints": {
            "health": "/health",
            "parse": "/parse",
        ...
        "note": "Для полного функционала установите Tesseract OCR"
    }
