...
app = FastAPI(title="Document Parser API (With OCR)", version="2.0")
...
try:
    ...
    print("Tesseract OCR недоступен")
...
async def parse_passport(image: Image.Image) -> Dict[str, Any]:
    ...
    2. Tesseract OCR (fallback) — если доступен
    """
    result = {
        "success": False,
    ...
    # Шаг 2: Tesseract OCR (fallback) - если доступен
    if TESSERACT_AVAILABLE:
        try:
            processed = preprocess_image(image)
            
            # Tesseract с русским и английским языками
    ...
    result.update({
        ...
        "confidence": 0,
        "data": {},
        "error": "MRZ не найден. OCR недоступен. Установите Tesseract OCR для полного функционала."
    })
    return result
...
async def parse_driver_license(image: Image.Image) -> Dict[str, Any]:
    ...
    # Шаг 2: OCR fallback - если доступен
    if TESSERACT_AVAILABLE:
        try:
            processed = preprocess_image(image)
    ...
    result.update({
        ...
        "confidence": 0,
        "data": {},
        "error": "PDF417 не найден. OCR недоступен. Установите Tesseract OCR для полного функционала."
    })
    return result
...
def health_check():
    return {
        "status": "ok", 
        "service": "document-parser-with-ocr",
...
def root():
    return {
        "message": "Document Parser API (With OCR)",
        "version": "2.0",
        "tesseract_available": TESSERACT_AVAILABLE,
        "endpoints": {
            "health": "/health",
            "parse": "/parse",
            "docs": "/docs",
            "test_client": "/static/test_client.html"
        },
        "note": "Tesseract OCR " + ("доступен" if TESSERACT_AVAILABLE else "недоступен")
    }
